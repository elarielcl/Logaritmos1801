...
\rhead{\includegraphics[scale=0.03]{\@logo}}
...
% COMMANDS TO ADD TEACHING ASSISTANTS
\newcounter{assistanttCnt}
\NewDocumentCommand\assistantt{ o m }{%
  \IfValueTF{#1}{%
    \expandafter\gdef\csname assistantt@\theassistanttCnt\endcsname{#2 \@email{#1}} 
  }{%
    \expandafter\gdef\csname assistantt@\theassistanttCnt\endcsname{#2} 
  }%
  \stepcounter{assistanttCnt}
}
\newcommand{\@Ayudante}{Ayudante\ifthenelse{\value{assistanttCnt} = 1}{}{s}:}
...
\renewcommand{\maketitle}{
  \begin{center}
    ...
    \begin{tabular}{rl}
      ...
      \ifthenelse{\value{assistanttCnt} = 0}{}{\\[0.2em]
        \forloop{cnt}{0}{\value{cnt} < \value{assistanttCnt}}{
          \ifthenelse{\value{cnt} = 0}{\@Ayudante}{\\} & \csname assistantt@\thecnt\endcsname 
        }
      }
    \end{tabular}
    }
